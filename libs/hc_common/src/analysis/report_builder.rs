...
use crate::analysis::{analysis::AnalysisReport, score::ScoringResults, session::Session};
		...
		|builder, output| {
			match output.as_ref() {
				...
				_ => {
					return Err(hc_error!(
						"phase name does not match {} analysis",
						crate::analysis::score::PR_CONTRIBUTOR_TRUST_PHASE
					))
				}
			}
		...
		|builder, output| {
			match output.as_ref() {
				...
				_ => {
					return Err(hc_error!(
						"phase name does not match {} analysis",
						crate::analysis::score::PR_MODULE_CONTRIBUTORS_PHASE
					))
				}
			}
