...
use crate::data::git::{Contributor, Diff, FileDiff, RawCommit};
use crate::{
...
fn date(input: &str) -> StdResult<DateTime<FixedOffset>, String> {
	DateTime::parse_from_rfc3339(input).map_err(|e| {
		format!(
...
fn commit(input: &str) -> IResult<&str, RawCommit> {
	...
	let (input, signer_name) = line(input)?;
	let (input, signer_key) = line(input)?;
	...
	let commit = RawCommit {
		...
		signer_name: match signer_name {
			"" => None,
			_ => Some(signer_name.to_owned()),
		},
		signer_key: match signer_key {
			"" => None,
			_ => Some(signer_key.to_owned()),
		},
	};
