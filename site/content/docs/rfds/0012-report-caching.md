...
and ref, Hipcheck is deterministic across runs. Therefore, we can help drive
down execution time of multi-target analyses in resource-constrained
environments such as Continuous Integration (CI) runners by allowing Hipcheck to
cache the report of previous runs and skip redundant execution.
...
hc cache report reviewed --id <UNIQUE_ID>
```
