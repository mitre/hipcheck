...
impl Drop for PrintTime {
    fn drop(&mut self) {
        println!("[TIMINGS]: {} returned after {} seconds.", self.location, (Instant::now() - self.start).as_secs_f64());
    }
}

macro_rules! print_scope_time {
    () => {
        $crate::benchmarking::PrintTime::new(format!("{}:{}:{}", module_path!(), line!(), column!()))
    };

    ($msg:literal) => {
        $crate::benchmarking::PrintTime::new(format!("{}:{}:{} ({})", module_path!(), line!(), column!(), $msg))
    };
}
