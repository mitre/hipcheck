...
pub trait ParallelTrackAsPhase: Sized + ParallelIterator {
    ...
    fn track_as_spinner_phase(self, name: impl Into<Arc<str>>) -> SpinnerPhaseTracker<Self> {
        SpinnerPhaseTracker { phase: SpinnerPhase::start(name), iter: self }
    }
    ...
    {
        ProgressPhaseTracker { phase: ProgressPhase::start(self.len() as u64, name), iter: self }
    }
    ...
    where Self: ExactSizeIterator
    {
        ProgressPhaseTracker { phase: ProgressPhase::start_bytes(self.len() as u64, name), iter: self }
    }
}
...
impl<S: Send, T: ParallelIterator<Item = S>> ParallelIterator for SpinnerPhaseTracker<T> {
    ...
    fn drive_unindexed<C: UnindexedConsumer<Self::Item>>(self, consumer: C) -> C::Result {
        let spinner_consumer = PhaseConsumer { base: consumer, phase: self.phase };
...
impl<I: IndexedParallelIterator> IndexedParallelIterator for SpinnerPhaseTracker<I> {
    ...
    fn drive<C: Consumer<Self::Item>>(self, consumer: C) -> <C as Consumer<Self::Item>>::Result {
        let consumer = PhaseConsumer { base: consumer, phase: self.phase };
    ...
    fn with_producer<CB: ProducerCallback<Self::Item>>(
        ...
        impl<T, CB: ProducerCallback<T>> ProducerCallback<T> for Callback<CB> {
            ...
            where
                P: Producer<Item = T>,
            {
                let producer = PhaseProducer { base, phase: self.phase };
...
impl<T, C: Folder<T>> Folder<T> for PhaseFolder<SpinnerPhase, C> {
    ...
    fn consume(self, item: T) -> Self {
        ...
        PhaseFolder { base: self.base.consume(item), phase: self.phase }
    }
    ...
    PhaseFolder<P, C::Folder>: Folder<T, Result = C::Result>,
{
    ...
    fn split_at(self, index: usize) -> (Self, Self, Self::Reducer) {
        ...
        (
            PhaseConsumer { base: left, phase: self.phase.clone() },
            PhaseConsumer { base: right, phase: self.phase },
            reducer,
        )
    }
