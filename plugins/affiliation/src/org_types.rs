...
impl Strategy {
    pub fn new_implicit(mode: Mode) -> Self {
        Self {
            mode,
            children: None,
        }
	...
	#[allow(dead_code)]
	pub fn pop(&mut self) -> Result<Option<StrategyChild>> {
		match self.children {
            Some(ref mut children) => {
                Ok(children.pop())
            },
	...
	pub fn orgs_to_analyze<'spec>(&self, full_list: &'spec [Org]) -> Result<Vec<&'spec Org>> {
		if let Some(specifiers) = &self.children {
            ...
            for specifier in specifiers {
                let mut addition = match specifier {
                    StrategyChild::StrategyCountry(country) => get_by_country(&country.0, full_list)
        ...
        Err(anyhow!("Cannot retrieve org specfiers from an implicit strategy"))
	}
}
...
impl OrgList {
	...
	pub fn push(&mut self, plugin: Org) {
		self.0.push(plugin);
	}

	#[allow(dead_code)]
	pub fn pop(&mut self) -> Option<Org> {
		self.0.pop()
	}
}
...
impl Org {
    ...
    pub fn with_capacity(name: String, country: String, capacity: usize) -> Self {
        Self {
            name,
            country,
	...
	#[allow(dead_code)]
	pub fn pop(&mut self) -> Option<Host> {
		self.children.pop()
	}
