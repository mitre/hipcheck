...
async fn entropy(engine: &mut PluginEngine, value: Target) -> Result<Vec<f64>> {
	...
	let policy_config = GLOBAL_POLICY_CONFIG.get().cloned().flatten();
...
static GLOBAL_POLICY_CONFIG: OnceLock<Option<PolicyExprConf>> = OnceLock::new();
...
impl Plugin for EntropyPlugin {
	...
	fn default_policy_expr(&self) -> Result<String> {
		match GLOBAL_POLICY_CONFIG.get() {
