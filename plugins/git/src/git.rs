...
// a lot of the data related to commits can be derived from a Vec<RawCommit>, so cache this and reuse it when possible
static ALL_RAW_COMMITS: OnceLock<Result<Vec<RawCommit>>> = OnceLock::new();

// initialize the repo so only one repository is used
fn initialize_repo<P>(repo_path: P) -> Result<Repository>
where
	P: AsRef<Path>,
{
	gix::discover(repo_path).context("failed to find repo")
}
...
fn get_commit_walker(repo: &Repository) -> Result<Walk<'_>> {
	let head_commit_id = repo.head_commit()?.id();
	let repo_walker = repo
		.rev_walk(Some(head_commit_id))
...
pub fn get_latest_commit(repo: &str) -> Result<Option<RawCommit>> {
	let repo = initialize_repo(repo)?;
	let mut commit_walker = get_commit_walker(&repo)?;
...
/// get all of the `RawCommit` from a repo **sorted from newest to oldest**
///
...
pub fn get_all_raw_commits<P>(repo: P) -> Result<Vec<RawCommit>>
where
	P: AsRef<Path>,
{
	let commits = ALL_RAW_COMMITS.get_or_init(|| {
		let repo = initialize_repo(repo)?;
		let commit_walker = get_commit_walker(&repo)?;
		let commits = walk_commits(&repo, commit_walker, &get_raw_commit, None)?;
		Ok(commits)
	});
	match commits {
		Ok(commits) => Ok(commits.clone()),
		Err(e) => Err(anyhow::Error::msg(e.to_string())),
	}
}

pub fn get_commits_from_date(repo: &str, cutoff_date: Timestamp) -> Result<Vec<RawCommit>> {
	let raw_commits = get_all_raw_commits(repo)?;
...
pub fn get_contributors(repo: &str) -> Result<Vec<Contributor>> {
	let commits = get_all_raw_commits(repo)?;
...
pub fn get_commit_diffs(repo: &str) -> Result<Vec<CommitDiff>> {
	let repo = initialize_repo(repo)?;
	let commit_walker = get_commit_walker(&repo)?;
