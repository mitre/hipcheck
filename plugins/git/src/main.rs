...
use crate::{
	...
	util::git_command::{get_commits_from_date, get_diffs, get_raw_commits},
};
use clap::Parser;
...
use util::git_command::{get_commit_diffs, get_commits};
...
fn local_raw_commits(repo: LocalGitRepo) -> Result<Vec<RawCommit>> {
	get_raw_commits(&repo.path).map_err(|e| {
...
async fn last_commit_date(_engine: &mut PluginEngine, repo: LocalGitRepo) -> Result<String> {
	let path = &repo.path;
	let commits = get_raw_commits(path).map_err(|e| {
		log::error!("failed to get raw commits: {}", e);
		Error::UnspecifiedQueryState
	})?;
...
async fn commits_from_date(
	...
	let commits_from_data = get_commits_from_date(path, timestamp).map_err(|e| {
		log::error!("failed to get raw commits from date: {}", e);
		Error::UnspecifiedQueryState
	})?;
	Ok(commits_from_data)
}
...
async fn contributors(_engine: &mut PluginEngine, repo: LocalGitRepo) -> Result<Vec<Contributor>> {
	let path = &repo.path;
	let raw_commits = get_raw_commits(path).map_err(|e| {
		log::error!("failed to get raw commits: {}", e);
		Error::UnspecifiedQueryState
	})?;

	let mut contributors: Vec<Contributor> = raw_commits
		.into_iter()
		.flat_map(|raw| [raw.author, raw.committer])
		.collect();
...
async fn commit_diffs(_engine: &mut PluginEngine, repo: LocalGitRepo) -> Result<Vec<CommitDiff>> {
	let diffs = get_commit_diffs(&repo.path).map_err(|e| {
		eprintln!("failed to get commit_diffs: {}", e);
		Error::UnspecifiedQueryState
	})?;
	Ok(diffs)
}
...
async fn batch_commits_for_contributor(
	...
	let commits: Vec<Commit> = raw_commits
		.iter()
		.map(|raw_commit| Commit::from(raw_commit.clone()))
		.collect();
...
async fn batch_contributors_for_commit(
	...
	let commit_views: Vec<CommitContributorView> = raw_commits
		...
		.enumerate()
		.map(|(i, raw)| {
			let commit = Commit::from(raw.clone());
