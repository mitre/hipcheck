...
impl NpmCommand {
		...
		S: AsRef<OsStr>,
	{
		...
		log_args(&npm_path.display().to_string(), args);
	...
	pub fn output(&mut self) -> Result<String> {
		...
		match String::from_utf8(output.stderr) {
			Ok(msg) if msg.is_empty().not() => {
				Err(anyhow!("(from npm) {} [{}]", msg.trim(), output.status))
			}
			_ => Err(anyhow!("npm failed [{}]", output.status)),
		}
	}
}
...
mod test {
	use super::*;
	use crate::util::command::check_version;
